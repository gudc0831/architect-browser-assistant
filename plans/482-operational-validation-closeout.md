...
| Browser validation | partially_passed | `architect-saas` | Operator login was completed on the Vercel branch alias. After DB migration, `/admin/knowledge` renders the Knowledge Admin shell; `/daily` reaches the authenticated app shell but still does not complete workspace loading. Stale Supavisor/Postgres idle sessions were cleared once, but the current deployment immediately refilled the session pool; `architect-saas` now caps Prisma/pg app pool usage pending redeploy. |
...
| Supabase stale idle session cleanup | temporary_only | Supabase MCP cleanup terminated 15 `postgres`/`Supavisor` idle sessions and immediate aggregate dropped that class to 1 idle session. An authenticated `/daily` reload then refilled the current deployment's pool back to 15 idle sessions, so the durable blocker is app runtime pool exhaustion on the deployed Git SHA. `architect-saas` now caps Prisma/pg pool usage pending redeploy and revalidation. |
...
Provider-backed embedding mutation remains blocked until embedding provider credentials and write-audit approval are supplied; the current migrated cloud DB has no `files` or `file_analysis_chunks` rows, so there is no backfill payload to process. `/admin/knowledge` authenticated UI signoff passed after migration, but `/daily` still does not complete workspace loading on the currently deployed branch alias because the app runtime refills the Supavisor session pool. Stale idle session cleanup was executed and verified as temporary; durable `/daily` recovery now requires deploying the `architect-saas` Prisma/pg pool cap patch and revalidating the operator session. Production legal-source import, external remote sync writes, Web Store upload, signed native-host installer release, and production readiness remain blocked until production SaaS origin/signing metadata and external approvals are supplied.
